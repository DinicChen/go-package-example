...
// bufio 包实现了带缓存的 I/O 操作. 它把一个 io.Reader 或者 io.Writer 对象封装
// 成为另一个对象(Reader或者Writer), 这个对象也实现了这个接口, 并提供缓冲和文
// 档读写的帮助.
package bufio
...
// ScanBytes 是用于 Scanner 类型的分割函数(符合 SplitFunc), 将每个字节作为一个 token 返回.
...
// ScanLines 是用于 Scanner 类型的分割函数(符合 SplitFunc), 将每一行文本去掉末尾的换
// 行标记作为一个 token 返回. 返回的行可以是空字符串. 换行标记为一个可选的回车后跟一个
// 必选的换行符. 正则表达式符号为 `\r?\n`. 最后一行即使没有换行符也会作为一个 token
// 返回.
...
// ScanRunes 是用于 Scanner 类型的分割函数(符合 SplitFunc), 将每个 utf-8 编码的 unicode
// 码值作为一个 token 返回. 返回的 rune 序列和 range 一个字符串的输出 rune 序列相同. 错
// 误的 utf-8 编码会翻译为 U+FFFD = "\xef\xbf\xbd", 但只会消耗一个字节. 调用者无法区分正
// 确编码的 rune 和错误编码的 rune.
...
// ScanWords 是用于 Scanner 类型的分割函数(符合 SplitFunc), 将每个被空白分割的单
// 词作为一个 token 返回. 不会返回空字符串. 空白符合 unicode.IsSpace 定义.
func ScanWords(data []byte, atEOF bool) (advance int, token []byte, err error)

// ReadWriter stores pointers to a Reader and a Writer. It implements
// io.ReadWriter.

// ReadWriter 存储输入输出指针. 它实现了 io.ReadWriter.
...
// NewReadWriter 分配新的 ReadWriter 来进行 r 和 w 的调度.
...
// NewReader 返回一个新的 Reader, 这个 Reader 的缓存大小是默认大小.
...
// NewReaderSize 返回了一个新的 Reader, 这个 Reader 的缓存大小至少为指定的大小. 如
// 果 io.Reader 参数已经是一个有足够大缓存的 Reader, 它就会返回这个底层 Reader.
func NewReaderSize(rd io.Reader, size int) *Reader

// Buffered returns the number of bytes that can be read from the current buffer.

// Buffered 返回当前缓存的可读字节数.
...
// Peek 读取指定字节数的数据, 这些被读取的数据不会从缓冲区中清除. 在下次读取之后, 本
// 次返回的字节切片会失效. 如果返回的字节数不足 n 字节, 则会同时返回一个错误说明原因.
// 如果 n 比缓冲区要大, 则错误为 ErrBufferFull.
