...
// 此处列出的条目其实并不在buildin包中，对它们的描述只是为了让godoc给该语言的特殊标识符提供文档。
package builtin
...
// iota 为预声明的标识符，它表示常量声明中（一般在括号中），当前常量规范的无类型化整数序数。
// 它从0开始索引。
...
// panic 内建函数停止当前Go程的正常执行。当函数 F 调用 panic 时，F 的正常执行就会立刻停止。
// 任何 F 的已延时函数都会按照一般的方式运行，接着 F 返回给其调用者。
// 对于其调用者 G，F 的请求行为如同对 panic 的调用，即终止 G 的执行并运行任何已延时函数。
// 这会持续到该Go程中所有函数都按相反的顺序停止执行之后。
// 此时，该程序会被终止，而错误情况会被报告，包括引发该恐慌的实参值。
// 此终止序列称为恐慌过程，并可通过内建函数 recover 控制。
...
// 该函数可以用于程序引导和调式阶段, 但是并不保证该函数始终有效.
...
// 在延迟函数（而不是任何被它调用的函数）中，执行 recover 调用会通过恢复正常的执行并取回传至 panic 调用的错误值来停止该恐慌过程序列。
// 若 recover 在延迟函数之外被调用，它将不会停止恐慌过程序列。
// 在此情况下，或当该Go程不在恐慌过程中时，或提供给 panic 的实参为 nil 时，recover 就会返回 nil。
// 因此 recover 的返回值就报告了该Go程是否在恐慌过程中。
...
// 其返回值即为对应的复数类型（complex64 对应 float32，complex128 对应 float64）。
...
// 不同的是，make 的返回类型与其参数相同，而非指向它的指针。
// 其具体结果取决于具体的类型：
//
//	切片：size 指定了其长度。
//		该切片的容量等于其长度。
...
//	映射：初始分配的创建取决于 size，但产生的映射长度为0。
//		size 可以省略，这种情况下就会分配一个小的起始大小。
//	信道：信道的缓存根据指定的缓存容量初始化。
...
// string 是所有8位字节的字符串集合。
// 习惯上用于代表以UTF-8编码的文本，但并不必须如此。
// string 可为空，但不为 nil。
// string 类型的值是不变的。
type string string
