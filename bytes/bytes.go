...
// IndexFunc 把 s 解释成 UTF-8 字节序列，并返回第一个满足 f(c) == true 的字符 c 的位置索引；
// 如果没有字符满足f，则返回-1。
...
// Bytes 返回缓冲中未读数据的切片, len(b.Bytes()) == b.Len().
// 如果中间没有调用其他方法, 修改返回的切片的内容会直接改变缓冲中的内容.
...
// Next 返回缓冲中未读取部分前 n 字节数据的切片, 并且移动读取位置, 就像调用了Read方法一样.
...
// Read 方法从缓冲中读取数据直到缓冲中没有数据或者读取了 len(p) 字节数据, 将读取的数据写入p.
// 返回值 n 是读取的字节数.
// 如果缓冲中完全没有数据可以读取并写入 p, 此时返回值 err 为 io.EOF, 否则 err 总是 nil.
